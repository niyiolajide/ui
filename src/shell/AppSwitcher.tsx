...
export default function AppSwitcher({ apps, currentKey }: { apps: AppInfo[]; currentKey?: string }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-64 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 shadow-elevated p-2 z-50">
          <p className="px-2 py-1 meta-label">Apps</p>
          <div className="grid grid-cols-2 gap-1">
            {apps.map((a) => {
              const Icon = ((Icons as unknown as Record<string, React.ComponentType<{ className?: string }>>)[a.icon || ''] || Icons.AppWindow)
